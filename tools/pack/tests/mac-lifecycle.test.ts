...
describe("startPackedMacApp", () => {
  it("accepts a clean launcher exit when the delegated desktop becomes healthy", async () => {
    ...
    try {
      ...
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "OpenComputer Design");
  ...
  it("uses the preview executable name for preview release namespaces", async () => {
    ...
    try {
      ...
      const paths = resolveMacPaths(config);
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "OpenComputer Design Preview");
