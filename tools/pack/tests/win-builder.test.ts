...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "OpenComputer Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "OpenComputer Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "OpenComputer Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "OpenComputer Design", "OpenComputer Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "OpenComputer Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "OpenComputer Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "OpenComputer Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "OpenComputer Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "OpenComputer Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "OpenComputer Design.exe"))).resolves.toMatchObject({
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "OpenComputer Design",
