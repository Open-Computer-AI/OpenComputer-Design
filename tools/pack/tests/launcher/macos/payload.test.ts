...
describe("tools-pack mac launcher payload archives", () => {
  it("builds a channel- and namespace-scoped payload manifest", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "OpenComputer Design Beta.app",
      channel: "beta",
      entry: {
  ...
  it.skipIf(process.platform !== "darwin")("creates a payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("OpenComputer Design Beta.app");
      expect(manifest.entry.executable).toBe("payload/OpenComputer Design Beta.app/Contents/MacOS/OpenComputer Design Beta");
