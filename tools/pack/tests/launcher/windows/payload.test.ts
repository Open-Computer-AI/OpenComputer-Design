...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "OpenComputer Design-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "OpenComputer Design-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "OpenComputer Design Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "OpenComputer Design Beta", "OpenComputer Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "OpenComputer Design-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "OpenComputer Design-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "OpenComputer Design-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "OpenComputer Design Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "OpenComputer Design.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "OpenComputer Design.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "OpenComputer Design.exe"),
      source: "namespace",
...
describe("tools-pack Windows launcher payload archives", () => {
  it("builds a channel- and namespace-scoped payload manifest", () => {
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/OpenComputer Design.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/OpenComputer Design.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "OpenComputer Design.exe"));
