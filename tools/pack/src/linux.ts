...
const PRODUCT_NAME = "OpenComputer Design";
...
async function writeAssembledApp(
  ...
  const packageJson = {
    ...
    author: "OpenComputer Design Team",
    repository: {
      type: "git",
      url: "https://github.com/Open-Computer-AI/OpenComputer-Design.git"
    }
  };
...
async function writeLinuxBuilderConfig(config: ToolPackConfig, paths: LinuxPaths): Promise<void> {
  ...
  const builderConfig: Record<string, unknown> = {
    ...
    linux: {
      ...
      synopsis: "OpenComputer Design",
      maintainer: "OpenComputer Design Contributors",
    },
...
export async function installPackedLinuxHeadless(config: ToolPackConfig): Promise<LinuxHeadlessInstallResult> {
  ...
  const script = [
    "#!/bin/sh",
    `# OpenComputer Design headless launcher — namespace: ${config.namespace}`,
