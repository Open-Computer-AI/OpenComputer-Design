...
export const en: Dict = {
  ...
  'useEverywhere.section.mcp.intro': 'OpenDesign ships with a Model Context Protocol server (`ocd mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover OpenDesign tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
