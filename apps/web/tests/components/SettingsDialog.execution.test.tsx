...
describe('SettingsDialog media providers interactions', () => {
  ...
  it('does not list remote media providers', () => {
    renderSettingsDialog(
      {
        mode: 'daemon',
        agentId: 'codex',
    ...
    expect(document.querySelectorAll('.media-provider-tabs .protocol-chip')).toHaveLength(0);
    expect(screen.queryByRole('tab', { name: /OpenAI/ })).toBeNull();
    expect(screen.queryByRole('tab', { name: /ElevenLabs/ })).toBeNull();
    expect(screen.queryByRole('tab', { name: /MiniMax/ })).toBeNull();
