  ...
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "OpenComputer Design's packaged runtime requires OD_DATA_DIR to be an absolute path.",
          "",
          `Configured value: ${odDataDir}`,
          "",
          "Set OD_DATA_DIR to an absolute path (for example, C:\\\\Users\\\\You\\\\OpenComputer Design on Windows or /Users/you/OpenComputer Design on macOS/Linux) and relaunch OpenComputer Design.",
        ].join("\n"),
        { title: "OpenComputer Design cannot start with this OD_DATA_DIR" },
      );
    }
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "OpenComputer Design's packaged runtime requires OD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
...
export function resolvePackagedNamespacePaths(
  ...
  // `~/Library/Application Support/OpenComputer Design Prerelease/` on mac. Custom
