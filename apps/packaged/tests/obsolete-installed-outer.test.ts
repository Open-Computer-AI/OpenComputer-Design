...
async function createMacInstalledOuter(): Promise<{
  ...
  if (process.platform === "win32") {
    const launchPath = "/Applications/OpenComputer Design.app";
    const executablePath = `${launchPath}/Contents/MacOS/OpenComputer Design`;
    return {
      executablePath,
  ...
  roots.push(root);
  const launchPath = join(root, "OpenComputer Design.app");
  const executablePath = join(launchPath, "Contents", "MacOS", "OpenComputer Design");
...
async function createWindowsInstalledOuter(
  executableName = "OpenComputer Design.exe",
): Promise<{
  executablePath: string;
  inspectInstalledOuterPath?: InspectInstalledOuterPath;
  launchPath: string;
  ...
  if (process.platform !== "win32") {
    const executablePath = `C:\\Program Files\\OpenComputer Design\\${executableName}`;
...
describe("createObsoleteInstalledOuterRetirement", () => {
  it("stops only the exact installed outer root and its descendants", async () => {
    ...
    const snapshots = [
      ...
      snapshot(102, 101, `${launchPath}/Contents/Frameworks/OpenComputer Design Helper.app/Contents/MacOS/OpenComputer Design Helper`),
      snapshot(103, 102, "helper-child"),
      snapshot(104, 1, `${executablePath} Helper`),
      snapshot(105, 1, "/unrelated/OpenComputer Design"),
      snapshot(900, 1, "/payload/OpenComputer Design.app/Contents/MacOS/OpenComputer Design"),
    ];
    const stopProcesses = stopMock();
    const logger = { info: vi.fn(), warn: vi.fn() };
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/OpenComputer Design.app/Contents/MacOS/OpenComputer Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/OpenComputer Design.app/Contents/MacOS/OpenComputer Design",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("does nothing outside an exact payload desktop process", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/installed/OpenComputer Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
  ...
  it("rejects a symlinked install executable before enumerating processes", async () => {
    ...
    if (process.platform === "win32") {
      launchPath = "/Applications/OpenComputer Design.app";
      const executablePath = `${launchPath}/Contents/MacOS/OpenComputer Design`;
      ...
      roots.push(root);
      launchPath = join(root, "OpenComputer Design.app");
      ...
      await symlink(target, join(executableDirectory, "OpenComputer Design"));
    }
    const listProcessSnapshots = vi.fn(async () => []);
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/OpenComputer Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger: { info: vi.fn(), warn: vi.fn() },
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/OpenComputer Design",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("refuses to stop an outer tree that contains the current payload", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      ...
      listProcessSnapshots: async () => [
        ...
        snapshot(900, 800, "/payload/OpenComputer Design"),
...
describe("Windows obsolete installed outer retirement", () => {
  it("stops the revalidated exact installed outer root and its current descendants", async () => {
    ...
    const firstSnapshots = [
      ...
      snapshot(202, 201, "OpenComputer Design.exe --type=gpu-process"),
      snapshot(203, 202, "OpenComputer Design.exe --type=utility"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\OpenComputer Design.exe"),
    ];
    const secondSnapshots = [
      snapshot(201, 1, `"${executablePath}"`),
      snapshot(202, 201, "OpenComputer Design.exe --type=gpu-process"),
      snapshot(206, 202, "OpenComputer Design.exe --type=renderer"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\OpenComputer Design.exe"),
    ];
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      ...
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "c:\\payload\\OpenComputer Design.exe",
      platform: "win32",
    }, { inspectInstalledOuterPath, listProcessSnapshots, stopProcesses });
  ...
  it("skips a reused root pid when the second snapshot no longer matches the executable", async () => {
    ...
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\OpenComputer Design.exe",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger: { info: vi.fn(), warn: vi.fn() },
      payloadDesktopProcess: true,
  ...
  it("rejects installed executables whose basename does not match the payload", async () => {
    const { inspectInstalledOuterPath, launchPath } = await createWindowsInstalledOuter("Not OpenComputer Design.exe");
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\OpenComputer Design.exe",
  ...
  it("refuses a revalidated outer tree that contains the current payload", async () => {
    ...
    const snapshots = [
      ...
      snapshot(900, 800, "C:\\payload\\OpenComputer Design.exe"),
    ];
  ...
  it("rejects a symlinked Windows executable before enumerating processes", async () => {
    const executablePath = "C:\\Program Files\\OpenComputer Design\\OpenComputer Design.exe";
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\OpenComputer Design.exe",
