...
function fakeRuntime(payloadDesktopProcess: boolean): PackagedLauncherRuntime {
  return {
    ...
    desktopExecutablePath: "/tmp/payload/OpenComputer Design Beta.app/Contents/MacOS/OpenComputer Design Beta",
    descriptor: {} as PackagedLauncherRuntime["descriptor"],
    electronNodeCommand: null,
    installedLaunchPath: "/Applications/OpenComputer Design Beta.app",
...
describe("payload desktop delegation", () => {
  ...
  it("forwards only the OS invite URL across an outer-to-payload cold start", () => {
    ...
    expect(findPackagedDeeplinkArg(["OpenComputer Design.exe", "--unrelated", deeplink])).toBe(deeplink);
    expect(findPackagedDeeplinkArg(["OpenComputer Design.exe", "--unrelated"])).toBeNull();
    const plan = planPackagedPayloadDesktopDelegation(fakeRuntime(false), {
      currentPid: 4321,
      forwardedArgs: ["OpenComputer Design.exe", "--unrelated", deeplink],
      timeoutMs: 60_000,
    });
