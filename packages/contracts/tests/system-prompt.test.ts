...
describe('DISCOVERY_AND_PHILOSOPHY (contracts copy) — TodoWrite plan item count', () => {
  ...
  it('uses a bare, self-contained Ask mode override that drops the discovery layer and charter', () => {
    ...
    expect(prompt).toContain('https://github.com/Open-Computer-AI/OpenComputer-Design');
    expect(prompt).toContain('https://tryopencomputer.com');
    expect(prompt).toContain('Inferno');
    expect(prompt).not.toContain('https://open-design.ai/');
    expect(prompt).not.toContain('https://discord.gg/');
    expect(prompt).not.toContain('Claude Design alternative');
